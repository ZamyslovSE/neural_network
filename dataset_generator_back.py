...
intersectionCount = 1 # Количество пересечений
...
def generateCenter(centers, varCount, radius):
...
def generatePoints(pointCount, classCount, varCount, radius):
    points = []
    centers = []
    for i in range(0,classCount):
        center = generateCenter(centers, varCount, radius)
        centers.append(center)
        print("center: ",center)
        points.append(generateClass(pointCount, varCount, center, radius))
    return points

visualize(generatePoints(pointCount, classCount, varCount, radius))